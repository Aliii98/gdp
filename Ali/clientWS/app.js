...
function droneDeployed(id){
    ...
    channel.bind('App\\Events\\NewDrone',function(data){
        ...
        if (data.header == 'get-status'){
            ...
            console.log(data);
            var heatmap = getHeaMapFromDB();
            setTimeout(getHeat, 2500, id, heatmap);
        }
        else if (JSON.parse(data.msg).header == 'Mission'){
            console.log(JSON.parse(data.msg));
        }
        else{ //mission
            console.log(JSON.parse(data.msg));
            // console.log(JSON.parse(data.msg.lat));
            storeMission(JSON.parse(data.msg));
        }
...
function startStatus(id){
    ...
    if (hdr == 'stop-status'){
        ...
        setTimeout(fetchStatus, 500, id);
        return Promise.delay(1*1000).then(() => startStatus(id)); //default 3s
    }
}

function storeMission(waypoint){
    // console.log(waypoint.lat);
        //INSERT INTO drone (lat, lng, alt, weight) VALUES ($1, $2, $3, $4);
            var sql = `INSERT INTO waypoints (lat, lng) VALUES (${waypoint.lat}, ${waypoint.lng})`;
